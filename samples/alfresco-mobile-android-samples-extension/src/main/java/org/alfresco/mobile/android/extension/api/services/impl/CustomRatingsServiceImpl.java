...
public class CustomRatingsServiceImpl extends OnPremiseRatingsServiceImpl implements CustomRatingsService
{
    ...
    private float computeMyRating(UrlBuilder url)
    {
        ...
        if (j.size() == 0 && j.get(OnPremiseConstant.RATINGS_VALUE) == null) { return -1; }
        ...
        if (j.get(OnPremiseConstant.RATINGS_VALUE) instanceof Map)
        {
            ...
            if (js == null || (js.size() == 0 && js.get(CustomConstant.STARSRATINGSCHEME_VALUE) == null)) { return -1; }
        ...
        else
        {
            ...
            if (js == null || js.isEmpty()) { return -1; }
            jso = (Map<String, Object>) js.get(0);
        }

        if (jso.size() != 0
                && jso.get(OnPremiseConstant.APPLIEDBY_VALUE) != null
                && session.getPersonIdentifier()
                        .equals(JSONConverter.getString(jso, OnPremiseConstant.APPLIEDBY_VALUE))) { return Float
                .parseFloat(JSONConverter.getString(jso, OnPremiseConstant.RATING_VALUE)); }
